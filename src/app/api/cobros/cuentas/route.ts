...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = await traerTodo<Record<string, unknown>>((desde, hasta) => {
      let q = ctx.supabase
    ...
    // Sin tope: un total calculado sobre un pedazo del padron es un numero
    // equivocado que nadie nota.
    const allRows = await traerTodo<Record<string, unknown>>((desde, hasta) =>
      ctx.supabase
        .from("cuentas_por_cobrar")
    ...
    const cobMes = await traerTodo<Record<string, unknown>>((desde, hasta) =>
      ctx.supabase
    ...
    let cobradoMes = 0;
    for (const r of cobMes) cobradoMes += Number(r.monto) || 0;

    // Cuentas parciales (cantidad) para el resumen.
    let parciales = 0;
    for (const r of allRows) if (r.estado === "parcial") parciales += 1;

    // Historial de cobros recientes (para la pestaña de cobros registrados).
    const histRows = await traerTodo<Record<string, unknown>>((desde, hasta) =>
      ctx.supabase
        .from("cobros_clientes")
