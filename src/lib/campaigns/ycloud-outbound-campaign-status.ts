...
/**
 * Instancia dedicada monocliente: la búsqueda de `chat_campaign_recipients` se limita
 * al schema propio de esta instancia. El baseline multi-tenant barría `public`,
 * `zentra_erp` y los schemas `er_*` / `erp_*` de otros clientes; eso permitía leer y
 * actualizar destinatarios de campañas ajenos. Aquí no se sale de `SUPABASE_APP_SCHEMA`.
 */
