/**
 * Límites de día / mes en zona horaria Paraguay (America/Asuncion, UTC-3 fijo,
...
export function asuncionRangeBoundsUtc(
  ...
  const start = desde ? new Date(`${desde}T00:00:00.000-03:00`).toISOString() : month.start;
  const end = hasta ? new Date(`${hasta}T23:59:59.999-03:00`).toISOString() : month.end;
  return { start, end };
}
...
export function asuncionMonthBoundsUtc(now: Date = new Date()): { start: string; end: string } {
  ...
  const start = new Date(`${year}-${mm}-01T00:00:00.000-03:00`);
  const nextY = month === 12 ? year + 1 : year;
  const nextM = month === 12 ? 1 : month + 1;
  const nextMM = String(nextM).padStart(2, "0");
  const end = new Date(`${nextY}-${nextMM}-01T00:00:00.000-03:00`);
...
export function asuncionMesBoundsUtc(mes: string): { start: string; end: string } {
  ...
  const start = new Date(`${y}-${mm}-01T00:00:00.000-03:00`);
  const nextY = m === 12 ? y + 1 : y;
  const nextM = m === 12 ? 1 : m + 1;
  const end = new Date(`${nextY}-${String(nextM).padStart(2, "0")}-01T00:00:00.000-03:00`);
  end.setMilliseconds(end.getMilliseconds() - 1);
  return { start: start.toISOString(), end: end.toISOString() };
}
